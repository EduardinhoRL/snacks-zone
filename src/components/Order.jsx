...
const Row = styled.div`
  ...
  border-radius: 10px;
  display: grid;
  grid-template-columns: 320px 120px 100px 250px 70px;
...
function Order({order, handleRefresh, setChange}) {

  const {_id, clientName, clientPhoneNumber, createdAt, completedAt, isCompleted, products, total} = order
  ...
  const handleClick = e => {
    ...
    putData(`${process.env.REACT_APP_API_URL}/orders/${_id}`, order)
    .then(() => {
  ...
  return (
    <Row>
          ...
          <ListProducts>
              ...
              {products.map(product => (
                <div>
                  ...
                  <Price><span>${product.total}</span></Price>
                </div>
              ))}
      ...
      <Fecha>
        <div>
          <span className="material-icons">pending_actions</span> {fecha[0]} / {hora[0]}
        </div>
      </Fecha>

      <Checkbox>
        <BtnCompleted onClick={handleClick} className={isCompleted ? 'completed' : null} >{isCompleted ? 'Completado' : 'Incompleto'} </BtnCompleted>
      </Checkbox>        
    </Row>
  );
}
